package br.com.fulltime.fullarm.core.message.handler.command.subcommand.sirenoff;

import br.com.fulltime.fullarm.core.message.command.subcommand.Subcommand;
import org.springframework.stereotype.Service;

@Service
public class SirenOffMessageHandlerImpl implements SirenOffMessageHandler {
    @Override
    public String subcommandToMessage(Subcommand subcommand) {
        return subcommand.getType().toString();
    }
}
